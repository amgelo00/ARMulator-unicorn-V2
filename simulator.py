...
try:
   ...
   print("hello cinnamon, capstone presente")
except ImportError:
   CAPSTONE_AVAILABLE = False
   print("hello cinnamon, capstone non presente")
...
class Simulator:
    ...
    def __init__(self, memorycontent, assertionTriggers, addr2line, pcInitValue=0):
       ...
       # Crea UnicornEmulator PRIMA di usarlo!
        self.emu = UnicornEmulator()
        self.emu.dichiarazione_var()

        self.emu.mu = Uc(UC_ARCH_ARM, UC_MODE_ARM)
        self.emu.ks = Ks(KS_ARCH_ARM, KS_MODE_ARM)
        self.regs = Registers(self.history)
        self.mem = Memory(self.history, memorycontent)
        # 🔴 SALVA IL BYTECODE ASSEMBLATO NEI COMPONENTI
        self.mem.data["CODE"] = bytearray(memorycontent.get("CODE", []))
        self.mem.data["DATA"] = bytearray(memorycontent.get("DATA", []))
        self.mem.data["INTVEC"] = bytearray(memorycontent.get("INTVEC", []))

        #print(f"✅ Bytecode caricato: CODE={len(self.mem.data['CODE'])} bytes, DATA={len(self.mem.data['DATA'])} bytes")
    ...
    def bytecodeToInstr(self):
        ...
        if self.currentInstr is not None:
             ...
             try:
                ...
                # 🔴 Fallback a Capstone per VFP/NEON/istruzioni non riconosciute
                try:
                    from capstone import Cs, CS_ARCH_ARM, CS_MODE_ARM
                    cs = Cs(CS_ARCH_ARM, CS_MODE_ARM)
                    instr_list = list(cs.disasm(self.fetchedInstr, self.regs[15] - self.pcoffset))
                    if instr_list:
                        instr = instr_list[0]
                        # Crea un placeholder per mostrare l'istruzione
                        class VFPInstr:
                            def explain(self, sim):
                                return f"{instr.mnemonic.upper()} {instr.op_str}", f"VFP/NEON instruction (executed by Unicorn)"
                            def saveState(self):
                                return None
                            def restoreState(self, state):
                                pass
                            # 🔴 QUESTI ATTRIBUTI MANCAVANO
                            def setBytecode(self, bytecode):
                                pass

                            nextAddressToExecute = -1
                            affectedRegs = (set(), set())
                            affectedMem = (set(), set())
                            
                            pcmodified = False                  
                        self.currentInstr = VFPInstr()
                        self.decoderCache[instrInt] = (self.currentInstr, None)
                    else:
                        self.currentInstr = None
                        self.errorsPending.append("execution", err.text)
                except:
                    # Se Capstone non è disponibile, mostra bytecode
                    self.currentInstr = None
                    self.errorsPending.append("execution", f"VFP/NEON instruction: {self.fetchedInstr.hex()}")
